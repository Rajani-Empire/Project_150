...
Engine::Engine() {
  ...
  setupText(&applesEatenText, mainFont, "Food 0", 28, Color::Blue);
...
void Engine::startTheGame() {
  ...
  applesEatenText.setString("Food " + to_string(applesEatenTotal));
  FloatRect currentLevelTextBounds = currentLevelText.getGlobalBounds();
  applesEatenText.setPosition(
      Vector2f(currentLevelTextBounds.left + currentLevelTextBounds.width + 20, -9));
...
void Engine::beginNextLevel() {
  ...
  FloatRect currentLevelTextBounds = currentLevelText.getGlobalBounds();
  applesEatenText.setPosition(
      Vector2f(currentLevelTextBounds.left + currentLevelTextBounds.width + 200, -60));
}
...
void Engine::run() {
  ...
  while (window.isOpen()) {
    ...
    if (currentGameState == GameState::PAUSED || currentGameState == GameState::GAMEOVER) {
      ...
      //sleep(milliseconds()); // sleep so we don't peg the CPU
      continue;
    }
